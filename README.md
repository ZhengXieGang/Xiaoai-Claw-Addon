...
插件运行在 OpenClaw Gateway 进程里，入口只负责注册服务、HTTP 路由和 `xiaoai_*` 工具。长期运行的监听、登录态、设备状态、音频 relay 和控制台都在插件自己的状态目录下工作，不写入用户的网络环境细节。

```mermaid
flowchart LR
    User[用户] --> Speaker[小爱音箱]
    Speaker <--> XiaomiCloud[小米账号 / 米家 / MiNA 云]
    XiaomiCloud <--> XiaomiClient[src/xiaomi-client.ts]
    OpenClaw[OpenClaw Gateway] --> Entry[index.ts]
    Entry --> Provider[src/provider.ts]
    Provider --> XiaomiClient
    Provider --> AuthPortal[src/auth-portal.ts]
    Provider --> Console[src/console-page.ts + assets/ui]
    Provider --> State[(插件状态 / token / console state)]
    Provider --> Relay[音频 relay / TTS bridge]
    Provider --> Agent[xiaoai 专属 OpenClaw agent]
...
```mermaid
sequenceDiagram
    participant S as 小爱音箱
    participant X as 小米云
    participant P as XiaoAI Cloud Plugin
    participant A as xiaoai agent
    participant T as xiaoai_* 工具
...
```mermaid
flowchart TB
    Release[Release zip / ClawHub 包] --> Package[package.json openclaw.extensions]
    Package --> Manifest[openclaw.plugin.json]
    Manifest --> Contracts[contracts.tools + toolMetadata.optional]
    Package --> Runtime[dist/index.js]
    Runtime --> Gateway[OpenClaw Gateway 加载插件]
    Gateway --> Service[xiaoai-cloud-listener 服务]
    Gateway --> Routes[/api/xiaoai-cloud HTTP 路由]
    Gateway --> Tools[xiaoai_* 工具注册]
    Install[install.sh / install.cmd] --> Configure[scripts/configure-openclaw-install.mjs]
    Configure --> Allow[plugins.allow / tools.alsoAllow / xiaoai agent]
    Configure --> Gateway
```

对照 OpenClaw 官方插件规范，本项目采用这些约束：
- `openclaw.plugin.json` 是冷启动发现入口，声明 `configSchema`、`contracts.tools` 和 `toolMetadata`，Gateway 不需要导入运行时代码就能知道插件拥有哪些工具。
- `package.json` 的 `openclaw.extensions` 指向构建后的 `dist/index.js`，Release zip、手动脚本安装和 ClawHub 安装都复用同一个运行时入口。
- `index.ts` 导出符合 `definePluginEntry` 形态的 entry 对象，但不把 OpenClaw SDK 作为运行时硬依赖，避免旧版 Gateway 或离线 Release 包加载失败。
- 所有 `xiaoai_*` 工具都按 optional 工具注册。安装脚本会显式写入插件 allowlist 和专属 `xiaoai` agent 的工具 allowlist，避免把真实设备动作暴露给未配置的 agent。
