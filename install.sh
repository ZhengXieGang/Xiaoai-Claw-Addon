...
IMPLEMENTATION="$SCRIPT_DIR/scripts/install/install.sh"
...
if [ -z "$archive" ]; then
  echo "Missing installer implementation: $IMPLEMENTATION" >&2
  echo "Place the complete Release bundle beside install.sh, or run it from the bundle root." >&2
  exit 1
fi

temp_dir=$(mktemp -d "${TMPDIR:-/tmp}/xiaoai-install-entry.XXXXXX")
...
for candidate in \
  ...
  "$temp_dir"
do
  if [ -f "$candidate/scripts/install/install.sh" ]; then
...
if [ -z "$bundle_root" ]; then
  echo "Failed to locate an installer in $archive" >&2
  exit 1
fi
export XIAOAI_PROJECT_ROOT="$bundle_root"
if [ -f "$bundle_root/scripts/install/install.sh" ]; then
  sh "$bundle_root/scripts/install/install.sh" "$@"
else
