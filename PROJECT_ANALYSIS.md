...
- fallback guard 以前是在首次 `silenceSpeaker()` 结束后才开始补偿
- 这会把补偿整体往后推，尤其在云端部署时更容易错过原生回复起播窗口
- 现在改成以“拦截开始时刻”为锚点调度
- 并把 `pause` 命令自身耗时单独记为 `pauseCommandEstimateMs`
- guard 的多次补偿点围绕当前设备的 `interceptLeadEstimateMs` 和 `pauseCommandEstimateMs` 来布置
...
- 一开始就尝试 `silenceSpeaker()`
- 如果这台设备最近校准结果是 `mixed` 或 `fallback-only`
- 就从“拦截开始时刻”立刻挂上保守 `pause guard`

但旧版校准流程之前不是这样：

- 它会先等可见状态或 fallback 估算结果
- 然后才进入后续补偿
- 对于看不到原生起播状态的设备，这个时间差足够让音箱先吐出几个字
...
- 只要本轮校准落到 `fallback-only` 风格
- 就按“拦截开始时刻”同步布置保守 `pause guard`
...
1. 配置值低于 `120ms` 时，控制台只通过 `toast` 提示风险，不往卡片里塞额外说明
